...
// Le chemin './data.js' est correct si data.js est dans le même dossier que script.js (ici: js/)
import { appData } from './data.js';
// Importe les fonctions de génération de PDF depuis pdf-generator.js
// Le chemin './pdf-generator.js' est correct si pdf-generator.js est dans le même dossier que script.js (ici: js/)
...
function populateSelect(selectElement, items, placeholder) {
    ...
    // La désactivation doit être basée sur l'absence d'items, pas juste leur longueur.
    selectElement.disabled = !items || items.length === 0;

    if (items) {
        items.forEach(item => {
            const option = document.createElement('option');
            // Assurez-vous que la valeur est toujours la clé unique ou le nom
            option.value = typeof item === 'object' ? item.nom || item.value : item;
            option.textContent = typeof item === 'object' ? item.nom || item.text : item; // Utilisez textContent pour l'affichage
...
function populatePhases() {
    ...
    selectPhase.value = currentSession.phase; // Tente de restaurer la sélection

    // Si une phase est déjà sélectionnée dans la session, populons les suivantes
    if (currentSession.phase) {
        populateActivites();
    } else {
        // Sinon, désactivez les menus suivants
...
function populateActivites() {
    ...
    // IMPORTANT : Vérifiez que l'activité chargée est bien une option valide pour la phase actuelle
    const isValidLoadedActivite = activites.some(a => a.nom === currentSession.activite);

    if (currentSession.activite && isValidLoadedActivite) {
        populateTaches();
    } else {
        // Réinitialiser les champs dépendants si l'activité n'est pas valide ou non sélectionnée
...
function populateTaches() {
    ...
    // IMPORTANT : Vérifiez que la tâche chargée est bien une option valide pour l'activité actuelle
    const isValidLoadedTache = taches.some(t => t === currentSession.tache);

    // Si une tâche est sélectionnée, populons tous les champs qui en dépendent
    if (currentSession.tache && isValidLoadedTache) {
        populateProblematiques();
        populateResultatsAttendus();
        populateCompetencesVisees();
        populateRessourcesOnDonne();
    } else {
        // Réinitialiser les champs dépendants si la tâche n'est pas valide ou non sélectionnée
        currentSession.problematique = '';
        currentSession.resultatAttendu = '';
        currentSession.competenceVisee = '';
        currentSession.ressourceOnDonne = '';
...
function populateProblematiques() {
    const selectedActiviteName = selectActivite.value; // Dépend de l'activité sélectionnée
...
function populateResultatsAttendus() {
    const selectedTacheName = selectTache.value; // Dépend de la tâche sélectionnée
...
function populateRessourcesOnDonne() {
    // Les ressources "On donne" ne semblent pas dépendre des sélections précédentes dans data.js,
    // donc on les peuple simplement avec toutes les ressources disponibles.
...
selectPhase.addEventListener('change', () => {
    ...
    // Réinitialise toutes les sélections dépendantes de la phase
    ...
    populateActivites();
    // Vide et désactive les menus suivants
    populateSelect(selectTache, [], "Sélectionner une tâche");
    populateSelect(selectProblematique, [], "Sélectionner une problématique");
    populateSelect(selectResultatAttendu, [], "Sélectionner un résultat attendu");
    populateSelect(selectCompetenceVisee, [], "Sélectionner une compétence visée");
...
selectActivite.addEventListener('change', () => {
    ...
    // Réinitialise toutes les sélections dépendantes de l'activité
    currentSession.tache = '';
    currentSession.problematique = '';
    currentSession.resultatAttendu = '';
    currentSession.competenceVisee = '';
...
selectTache.addEventListener('change', () => {
    ...
    // Réinitialise toutes les sélections dépendantes de la tâche
    currentSession.problematique = '';
    currentSession.resultatAttendu = '';
    currentSession.competenceVisee = '';
    currentSession.ressourceOnDonne = '';
...
function loadSessionFromHistorique(index) {
    if (index >= 0 && index < historiqueSessions.length) {
        ...
        // Met à jour l'UI avec les données chargées en suivant l'ordre des dépendances
        selectPhase.value = currentSession.phase;
        populateActivites(); // Doit repopuler activites AVANT de définir sa value
        selectActivite.value = currentSession.activite;
        populateTaches(); // Doit repopuler taches AVANT de définir sa value
        selectTache.value = currentSession.tache;

        // Puis peuple les autres en cascade
        populateProblematiques();
        selectProblematique.value = currentSession.problematique;
        populateResultatsAttendus();
        selectResultatAttendu.value = currentSession.resultatAttendu;
...
function renderHistorique() {
    ...
    if (historiqueSessions.length === 0) {
        historiqueList.innerHTML = '<li style="text-align: center; color: var(--dark-grey);">Aucune séance sauvegardée pour le moment.</li>';
        return;
    }
...
function clearAllSelections() {
    ...
    selectPhase.value = '';
    // Vide et désactive tous les menus secondaires
    populateSelect(selectActivite, [], "Sélectionner une activité");
    populateSelect(selectTache, [], "Sélectionner une tâche");
    populateSelect(selectProblematique, [], "Sélectionner une problématique");
    populateSelect(selectResultatAttendu, [], "Sélectionner un résultat attendu");
